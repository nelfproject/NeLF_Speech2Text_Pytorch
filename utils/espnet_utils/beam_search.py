...
from .scorer_interface import PartialScorerInterface, ScorerInterface
...
class BeamSearch(torch.nn.Module):
    ...
    def __init__(
        ...
        task_token: int = None,
        task_labels: Optional[bool] = False,
        task_labels_v2: Optional[bool] = False,
        ...
        self.task_labels_v2 = task_labels_v2
    ...
    def init_hyp(self, x: torch.Tensor) -> List[Hypothesis]:
        ...
        if self.hyp_primer is not None:
            init_yseq = torch.tensor(self.hyp_primer, device=x.device)
        elif self.task_labels:
            init_yseq = torch.tensor([self.task_token, self.sos], device=x.device)
        elif self.task_labels_v2:
            init_yseq = torch.tensor([self.sos, self.task_token], device=x.device)
        else:
            init_yseq = torch.tensor([self.sos], device=x.device)
    ...
    def forward(
        ...
        for i in range(maxlen):
            ...
            logging.debug("GPU usage: %.2f GB" % (torch.cuda.memory_allocated()/(1024**3)))
