...
class MultiDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask,
                memory_multi, memory_mask_multi, cache=None):
        ...
        if self.concat_after:
            x_concat = torch.cat(
                (x, self.multi_attn(x, memory_multi, memory_multi, memory_mask_multi)), dim=-1
            )
            x = residual + self.concat_linear3(x_concat)
        else:
            x = residual + self.dropout(self.multi_attn(x, memory_multi, memory_multi,
                                                        memory_mask_multi))
        if not self.normalize_before:
            x = self.norm3(x)
        ...
        return x, tgt_mask, memory, memory_mask, memory_multi, memory_mask_multi
