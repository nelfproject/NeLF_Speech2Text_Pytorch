...
class BatchBeamSearch(BeamSearch):
    ...
    def init_hyp(self, x: torch.Tensor) -> BatchHypothesis:
        ...
        if self.hyp_primer is not None:
            init_yseq = torch.tensor(self.hyp_primer, device=device)
        elif self.task_labels:
            init_yseq = torch.tensor([self.task_token, self.sos], device=device)
        elif self.task_labels_v2:
            init_yseq = torch.tensor([self.sos, self.task_token], device=device)
        else:
            init_yseq = torch.tensor([self.sos], device=device)
